...
parser.add_argument('--adaptation', dest='load', action='store_true', help='adaptation mode')
parser.add_argument('--target_data', type=str, default='dataset/target.txt')
...
if opt['adapation']:
    target_file = opt['target_data']
else: taget_file = None

# load data
print("Loading data from {} with batch size {}...".format(opt['data_dir'], opt['batch_size']))
train_batch = DataLoader(opt['data_dir'] + '/MedCaus_train.txt', opt['batch_size'], opt, vocab, target_file, evaluation=False)
dev_batch = DataLoader(opt['data_dir'] + '/MedCaus_dev.txt', opt['batch_size'], opt, vocab, target_file ,  evaluation=True)
...
for epoch in range(1, opt['num_epoch']+1):
    train_loss = 0
    for i, batch1 in enumerate(train_batch):
        start_time = time.time()
        global_step += 1
        loss = trainer.update(batch1, i, tarin_batch.diff)
        train_loss += loss
    ...
    for i, batch in enumerate(dev_batch):
        metrics += trainer.predict(batch)
        features += trainer.get_feature(batch)
    
    p, total_predict, total_entity = metrics[0], metrics[1], metrics[2]
    dev_p = p * 1.0 / total_predict * 100 if total_predict != 0 else 0
    dev_r = p * 1.0 / total_entity * 100 if total_entity != 0 else 0
    ...
    dev_p_p = p_p * 1.0 / total_predict_p * 100 if total_predict_p != 0 else 0
    dev_r_p = p_p * 1.0 / total_entity_p * 100 if total_entity_p != 0 else 0
    dev_f1_p = 2.0 * dev_p_p * dev_r_p / (dev_p_p + dev_r_p) if dev_p_p != 0 or dev_r_p != 0 else 0
    
    
    
    print("[%d epoch] Precision_e: %.2f, Recall_e: %.2f, F1_e: %.2f, Precision_p: %.2f, Recall_p: %.2f, F1_p: %.2f" % (epoch, dev_p, dev_r, dev_f1, dev_p_p, dev_r_p, dev_f1_p), flush=True)
    dev_score = dev_f1_p
