...
from utils.constant import PAD_TOKEN, START,STOP
from typing import Tuple
from overrides import overrides

from utils import constant
from utils.crf_utils import log_sum_exp_pytorch
...
class LinearCRF(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.label_size = opt['num_tag']

        #self.use_char = config.use_char_rnn
        #self.context_emb = config.context_emb

        self.label2idx = constant.TAG_TO_ID
        self.labels = dict([(v,k) for k,v in self.label2idx.items()])
    ...
    def forward_unlabeled(self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor) -> torch.Tensor:
        ...
        alpha = make_cuda(torch.zeros(batch_size, seq_len, self.label_size))
    ...
    def backward(self, lstm_scores: torch.Tensor, word_seq_lens: torch.Tensor) -> torch.Tensor:
        ...
        beta = make_cuda(torch.zeros(batch_size, seq_len, self.label_size))
        ...
        perm_idx = torch.zeros(batch_size, seq_len)
    ...
    def forward_backward(self, lstm_scores: torch.Tensor, word_seq_lens: torch.Tensor) -> torch.Tensor:
        ...
        perm_idx = make_cuda(torch.zeros(batch_size, seq_len))
        for batch_idx in range(batch_size):
            perm_idx[batch_idx][:word_seq_lens[batch_idx]] = torch.range(word_seq_lens[batch_idx] - 1, 0, -1)
        perm_idx = perm_idx.long()
        for i, length in enumerate(word_seq_lens):
    ...
    def viterbi_decode(self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        sentLength = all_scores.shape[1]
        # sent_len =
        scoresRecord = make_cuda(torch.zeros([batchSize, sentLength, self.label_size]))
        idxRecord = make_cuda(torch.zeros([batchSize, sentLength, self.label_size], dtype=torch.int64))
        mask = make_cuda(torch.ones_like(word_seq_lens, dtype=torch.int64))
        startIds = make_cuda(torch.full((batchSize, self.label_size), self.start_idx, dtype=torch.int64))
        decodeIdx = make_cuda(torch.LongTensor(batchSize, sentLength))
